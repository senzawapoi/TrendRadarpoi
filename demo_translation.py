...
def demo_bilingual_translation():
    """演示双语翻译功能"""
    print("\n" + "=" * 60)
    print("【演示 3】双语翻译功能")
    print("=" * 60)
    ...
    mixed_news_list = [
        ...
        },
        {
            ...
            "summary": "投资者对通胀数据反应积极，股市大幅上涨。",
            "source": "财经新闻网"
        }
    ]
    
    print(f"\n待翻译新闻数量：{len(mixed_news_list)} 条\n")
    ...
    if result.get("success"):
        ...
        print(f"总数：{result['total']}")
        print(f"实际翻译数：{result['translated_count']}")
        print(f"双语模式：{result.get('bilingual_mode', False)}\n")
        
        for i, news in enumerate(result['translated_news'], 1):
            print(f"[新闻 {i}] 来源：{news['source']}")
            print(f"  原标题：{news['title']}")
            
            # 英文翻译
            if 'title_english' in news and news['title_english']:
                print(f"  英文翻译：{news['title_english']}")
            
            # 中文翻译
            if 'title_translated' in news and news['title_translated']:
                print(f"  中文翻译：{news['title_translated']}")
            
            if 'summary' in news:
                summary = news['summary']
                print(f"  原摘要：{summary[:60]}..." if len(summary) > 60 else f"  原摘要：{summary}")
            
            # 摘要翻译
            if 'summary_english' in news and news['summary_english']:
                summary_en = news['summary_english']
                print(f"  英文摘要：{summary_en[:60]}..." if len(summary_en) > 60 else f"  英文摘要：{summary_en}")
            
            if 'summary_translated' in news and news['summary_translated']:
                summary_zh = news['summary_translated']
                print(f"  中文摘要：{summary_zh[:60]}..." if len(summary_zh) > 60 else f"  中文摘要：{summary_zh}")
