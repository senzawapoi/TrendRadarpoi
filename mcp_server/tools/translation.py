...
{text}"""
    ...
    def translate_news_list(
        ...
        Args:
            ...
            source: 源语言
            target: 目标语言
            skip_if_chinese: 是否跳过已包含中文的内容（仅当目标为中文时有效）
            bilingual: 是否启用双语翻译（同时提供中英文翻译）
        ...
        for news in news_list:
            ...
            for field in fields:
                ...
                if bilingual:
                    # 翻译到中文
                    if detected_lang != "zh":
                        result_zh = self.translate_text(text, detected_lang, "zh")
                    ...
                    if detected_lang != "en":
                        ...
                        if result_en.get("success"):
                            translated_item[f"{field}_english"] = result_en["translated_text"]
                            item_translated = True
                else:
                    # 单语翻译模式
